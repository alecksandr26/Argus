...
`SIMULATOR_SCENARIO_WEIGHTS` (default 55% `normal` / 25% `drowsy_escalation` / 20% `panic` — with
`SIMULATOR_TRUCK_COUNT >= 4` you'll typically see at least one of each, but it's a weighted draw,
not a guaranteed round-robin):
...
| `SIMULATOR_TRUCK_COUNT` | `4` | how many virtual trucks to run |
...
| `SIMULATOR_SCENARIO_WEIGHTS` | `normal=0.55,drowsy_escalation=0.25,panic=0.20` | relative weights for the random per-truck profile draw |
| `SIMULATOR_MEDIUM_BLIP_PROBABILITY` | `0.1` | baseline chance of a `medium` vigilance blip on an otherwise-quiet tick |
| `SIMULATOR_RESET` | `true` | delete previously-created `SIM-*` fleet before provisioning fresh |
| `SIMULATOR_USE_OSRM` | `false` | real road-following routes instead of a straight line — see below |
...
By default a virtual truck's position is a straight-line interpolation between its origin and
destination city. Turning on `SIMULATOR_USE_OSRM` makes it walk the *real* road route instead,
via a local OSRM instance — but this needs a one-time setup step first, since it requires a real
Mexico OSM extract (~150MB) preprocessed into a routable graph:

```bash
cd src/simulator-argus
./scripts/setup_osrm.sh              # once — downloads + preprocesses into ./osrm-data/ (gitignored)
docker compose --profile osrm up -d osrm
SIMULATOR_USE_OSRM=true docker compose up --build simulator-argus
```

(Layering onto the root stack instead: run `setup_osrm.sh` and the `osrm` service from here, then
`SIMULATOR_USE_OSRM=true OSRM_BASE_URL=http://localhost:5001 docker compose --profile simulator up simulator-argus`
from the repo root.)

If OSRM isn't running yet, or the extract hasn't been preprocessed, `osrm_client.py` logs a
warning and falls back to the straight-line route for that truck — `SIMULATOR_USE_OSRM=true`
never blocks the simulator from starting. This is a separate, simulator-only use of OSRM for
generating realistic demo tracks — it isn't the backend/frontend's own route+ETA feature, which
remains deferred future work per the root `CLAUDE.md`.
